    ...
    <div className="space-y-5">
      ...
      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        {[
          ...
          <div key={String(label)} className="rounded-xl border border-line bg-surface px-4 py-3">
            ...
            <p className={`num mt-1 text-xl font-semibold ${color}`}>{Number(value).toLocaleString()}건</p>
          </div>
        ))}
      </div>
      {result && (
        <Panel title={`Data Quality ${result.score}%`} description="업로드 검증 항목">
          <div className="grid gap-2 sm:grid-cols-2 xl:grid-cols-3">
            {result.qualityChecks.map((item) => (
              <div key={item.label} className="flex justify-between rounded-lg border border-line px-3 py-2.5 text-sm">
                <span>{item.label}</span>
                <span className={`num ${item.count > 0 ? 'text-danger' : 'text-ok'}`}>{item.count}건</span>
              </div>
            ))}
          </div>
        </Panel>
      ...
      <Panel title="분석 반영 현황">
        <p className="text-sm text-muted">
          ...
          {analytics.summary.totalQty.toLocaleString()} · 부적합률 {analytics.summary.failRate.toFixed(2)}% ·
          제외 {analytics.summary.excludedCount.toLocaleString()}건
        </p>
      </Panel>
      <Panel title="이슈 행 미리보기" description="오류는 업로드 차단, 경고는 확인 후 저장, #N/A는 분석 제외">
              ...
              {records
                ...
                .map((r) => (
                  <tr key={r.id} className="border-b border-line/70">
                    ...
                    <td className="px-2 py-2.5 text-muted">{r.issues.join(', ') || '-'}</td>
                  </tr>
                ))}
            </tbody>
          </table>
