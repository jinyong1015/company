...
function aggregateDefects(records: InspectionRecord[]): DefectType[] {
  ...
  for (const r of records) {
    ...
    if (entries.length === 0 && r.fail > 0) {
      map.set(
        r.mainDefect || "기타",
        (map.get(r.mainDefect || "기타") ?? 0) + r.fail,
      );
      continue;
    }
...
function buildMonthlyTrends(
  ...
  const months = fillFullYear
    ? eachMonth(
        ...
        new Date(start.getFullYear(), 11, 1),
      )
    : eachMonth(start, end);

  return months.map(({ key, label }) =>
    toTrendPoint(label, byMonth.get(key) ?? []),
  );
}
...
function buildDefectTypes(
  ...
  const prevMap = new Map(
    aggregateDefects(previous).map((d) => [d.name, d.count]),
  );
...
function buildWorkerProductUph(
  records: InspectionRecord[],
): WorkerProductUph[] {
  const map = new Map<string, InspectionRecord[]>();
  for (const r of records) {
...
export function analyzeRecords(
  ...
  const previous = multiFiltered.filter((r) =>
